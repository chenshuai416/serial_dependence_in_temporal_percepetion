function  [pvalue_c, pvalue_z] = permutation(X,nperms)
...
for i_perm = 1:nperms
test = X;
test(:,8) = Shuffle([zeros(length(X_same),1);ones(length(X_short),1);2*ones(length(X_long),1)]);
[~,PSE_same,~]=fitpsy(test(test(:,8)==0,5:6));
[~,PSE_short,~]=fitpsy(test(test(:,8)==1,5:6)); %short=1
[~,PSE_long,~]=fitpsy(test(test(:,8)==2,5:6));  %long=2
